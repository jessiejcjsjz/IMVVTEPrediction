...
name<-c()
if (args == 1) {name = 'imv_whole_set1.rda'}
if (args == 2) {name = 'imv_whole_set2.rda'}
if (args == 3) {name = 'imv_whole_set3.rda'}
if (args == 4) {name = 'imv_whole_set4.rda'}
if (args == 5) {name = 'imv_whole_set5.rda'}
...
for(k in 1:10) {
  ...
  test<-data.matrix(my.data[which(my.data$CI_ID_t0_t0%in%choose.id), -1:-2])
  train<-data.matrix(my.data[-which(my.data$CI_ID_t0_t0%in%choose.id), -1:-2])
  ...
  xgboost.m<- xgboost(
    data = data.matrix(train[,-grep('IMV_t1',colnames(train))]),
    label = train[,grep('IMV_t1',colnames(train))],
    max.depth = 5,
    eta = 1,
    nround = 2,
  ...
  pred.xg<- predict(xgboost.m, data.matrix(test[,-grep('IMV_t1',colnames(test))]))

  #result.r<-data.frame(actual = test$IMV_t1, predict = pred.xg)
  result.r<-data.frame(actual = test[,grep('IMV_t1',colnames(test))], predict= pred.xg)
...
filename <-paste0('10folds_xg_imv_health',args,'.rda')
