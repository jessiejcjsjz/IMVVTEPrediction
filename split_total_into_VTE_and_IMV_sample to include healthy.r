...
#vte,imv,control,control_sample,seed=12345


i <- 1
#get subdata for those samples with Yi
vte_idlist<-c(idlist[[1]],sample(idlist[[4]], length(idlist[[1]])*9))

#begin to select the data
datai <- modelfile[which(claim[,1]%in%as.numeric(vte_idlist)),]
datai <- as.data.table(as.matrix(datai))
colnames(datai) <- colidlist

#Claimi and ID information for Yi, coding T
claimi <- claim[claim[,1]%in%as.numeric(vte_idlist),]
...
save(data0,file='sample_vte_wholehealth.rda')
...
i <- 2
#get subdata for those samples with Yi
imv_idlist<-c(idlist[[2]],sample(idlist[[4]], length(idlist[[2]])*5))

#begin to select the data
datai <- modelfile[which(claim[,1]%in%as.numeric(imv_idlist)),]
datai <- as.data.table(as.matrix(datai))
colnames(datai) <- colidlist

#Claimi and ID information for Yi, coding T
claimi <- claim[claim[,1]%in%as.numeric(imv_idlist),]
claimi <- cbind(claimi,ordert1=1)
#need to order the cliam by id
for(k in 2:nrow(claimi)){
	if(claimi[k,1]==claimi[k-1,1]){
...
data0$IMV_t1 <- ifelse(is.na(as.numeric(data1$IMV)),0,1)

save(data0,file='sample_imv_wholehealth.rda')